...
const dev = argv.options.dev || false,
      downloadUrl = dev ? 'https://github.com/wbrowar/WB-Starter/archive/dev.zip' : 'https://github.com/wbrowar/WB-Starter/archive/main.zip',
      npmInstaller = argv.options.installer ? argv.options.installer : 'npm',
      localRepo = argv.options.local ? argv.options.local.endsWith(`/`) ? argv.options.local.slice(0, -1) : argv.options.local : false,
      pkg = require(`../package.json`),
      unzippedDirName = dev ? 'dev' : 'main',
...
function run() {
    const questions = [{
        ...
        default: true,
        when: (answers) => {
            log('dump', _handleFromAnswers(answers), true);
            return fs.existsSync(`./${ _handleFromAnswers(answers) }`);
        },
    }];
    ...
    inquirer.prompt(questions).then((answers) => {
        ...
        if (answers.enableInstall ? answers.enableInstall : true) {
            const name = _handleFromAnswers(answers),
            ...
            downloadFromUrl(downloadUrl, () => {
                ...
                if (localRepo) {
                    ...
                    if (fs.pathExists(`${localRepo}/_starter`)) {
                        fs.copySync(`${localRepo}/_starter`, `${projectDir}/_starter`);
                        log('verbose', `Copied ${localRepo}/_starter to ${projectDir}/_starter`, verbose);
                    }
                    if (fs.pathExists(`${localRepo}/README.md`)) {
                        fs.copySync(`${localRepo}/README.md`, `${projectDir}/README.md`);
                        log('verbose', `Copied ${localRepo}/README.md to ${projectDir}/README.md`, verbose);
                    }
                } else {
                    if (fs.pathExists(`SETUP/WB-Starter-${ unzippedDirName }/_install`)) {
                        verboseExec(`mv SETUP/WB-Starter-${ unzippedDirName }/_install ${projectDir}/SETUP/_install`, verbose);
                        log('verbose', `Moved _install`, verbose);
                    }
                    if (fs.pathExists(`SETUP/WB-Starter-${ unzippedDirName }/_starter`)) {
                        verboseExec(`mv SETUP/WB-Starter-${ unzippedDirName }/_starter ${projectDir}/_starter`, verbose);
                        log('verbose', `Moved _starter`, verbose);
                    }
                    if (fs.pathExists(`SETUP/WB-Starter-${ unzippedDirName }/README.md`)) {
                        verboseExec(`mv SETUP/WB-Starter-${ unzippedDirName }/README.md ${projectDir}/README.md`, verbose);
                        log('verbose', `Moved package.json`, verbose);
                    }
                }
                ...
                const installCommand = `node ./install.js --handle='${ name }' --project-dir='${ projectDir }' --version='${pkg.version}' ${ dev ? '--dev' : '' } ${ verbose ? '--verbose' : '' }`;
...
function _handleFromAnswers(answers) {
    return answers.clientCode.toLowerCase() + '-' + answers.projectName.toLowerCase();
}
