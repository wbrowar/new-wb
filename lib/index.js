...
const npmInstallCommand = argv.options.installer ? argv.options.installer : 'npm install';
...
function run() {
    const questions = [{
        ...
        type: 'confirm',
        name: 'enableInstall',
        message: 'Continue with install in existing directory? This is probably not a good idea 😬',
        default: true,
        when: (answers) => {
    ...
    inquirer.prompt(questions).then((answers) => {
        ...
        if (answers.enableInstall ? answers.enableInstall : true) {
            ...
            downloadFromUrl(downloadUrl, () => {
                ...
                verboseExec(npmInstallCommand, verbose);
...
function verboseExec(command, verbose = false, showOutput = false) {
    if (verbose) {
      log('running', command);
